...
import { DEFAULT_CHART_CONFIG, DEFAULT_CHART_DATA } from "./widget-defaults";

interface EditChartWidgetSheetProps {
  widget: Widget;
}

export function EditChartWidgetSheet({
  widget,
}: Readonly<EditChartWidgetSheetProps>) {
  ...
  const getInitialData = () => {
    if (
      ...
      return JSON.stringify(DEFAULT_CHART_DATA, null, 2);
    }
  ...
  const getInitialConfig = () => {
    if (
      ...
      return JSON.stringify(DEFAULT_CHART_CONFIG, null, 2);
    }
  ...
  const handleOpenChange = (newOpen: boolean) => {
    if (newOpen) {
      ...
      if (
        ...
        setData(JSON.stringify(DEFAULT_CHART_DATA, null, 2));
      } else {
      ...
      if (
        ...
        setConfig(JSON.stringify(DEFAULT_CHART_CONFIG, null, 2));
      } else {
